...
function appendToFile(now, string) {
	var timestamp = String(now.format('YYYY/MM/DD HH:mm:ss '));
	fs.appendFileSync("../log/gui.txt", "\r\n"+timestamp+string);
}

function fileExist(now) {
	var timestamp = String(now.format('YYYY/MM/DD HH:mm:ss '));
	// fs.access("../log/"+filename+".txt", fs.F_OK, function (err) {
	fs.access("../log/gui.txt", fs.F_OK, function (err) {
		if(err) {	// if not exist gui.txt, create new gui.txt.
			var message = "This file was created at " + timestamp + "\r\n";
			fs.writeFileSync("../log/gui.txt", message);
		}
	});
}

function checkFileCreatedTime(now) {
	fs.stat('../log/gui.txt', function (err, stat) {
	    if (!err) {
	        if (stat.isFile()) {
				// compare two timestamp, file creation date and now
				var bd = moment(stat.birthtime);
				if (now.diff(bd, "days") >= 7) {	// if this file is created 7 days before, change filename and create a new templog file
					renameAndSave(now)
				}
			}
		}
...
function renameAndSave(now) {
	...
	fs.renameSync('../log/gui.txt', '../log/'+ filename +'_gui.txt'); 
}
